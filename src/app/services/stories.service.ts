...
export class StoriesService {
  ...
  getPrevious() {
    var index = this.lastStory > 0 ? this.lastStory - 1 : this.pageURLs.length - 1;
  ...
  getNext() {
    var index = this.lastStory + 1 < this.pageURLs.length - 1 ? this.lastStory + 1 : 0;
    var newPageURL = this.pageURLs[index];
    this.lastStory = index;
    return newPageURL;
  }
